...
| App's DB connection string & API key | Read from a pre-existing Kubernetes Secret (`resilientops-db-credentials`), created out-of-band with `kubectl create secret` — **not** a Helm value, so it never appears in `helm --set`, shell history, or Helm release metadata. | Azure Key Vault Provider for Secrets Store CSI Driver, or migrate the app to Postgres AAD authentication (below) and drop the password entirely. |
| ACI smoke-test path's ACR credentials | ACR admin account enabled (AD-010) — a shared username/password stored as Terraform-managed secure environment variables on the container group. | Managed identity + `AcrPull` role, matching the AKS path — not used here because ACI's identity-based ACR pull support is more limited than AKS's; acceptable for a throwaway smoke-test rig, not for anything longer-lived. |
